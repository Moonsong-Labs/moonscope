	...
	private constructor() {
		...
		if (check !== true) {
			throw new Error(
				`${check.join(
					",",
				)} environment variables are not defined, cannot connect.`,
			);
		}

		const connectionString = `postgres://${Bun.env.PGUSER}:${encodeURIComponent(
			Bun.env.PGPASSWORD || "error",
		)}@${Bun.env.PGHOST}:${Bun.env.PGPORT}/${Bun.env.PGDATABASE}`;
...
function checkEnvVars(): true | ConnectionEnvVars[] {
	const missingEnvVars = requiredEnvVars.filter(
		(envVar) => Bun.env[envVar] === undefined,
	);
